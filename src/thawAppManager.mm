...
void thawAppManager::update(){
	...
	while(receiver.hasWaitingMessages()){
        ...
        if(m.getAddress() == "/client/rgb"){
            ...
            if(!pause){
                float smooth;
                if(m.getArgAsFloat(7)<150){
                    
                    smooth = 0.6;
                    distance = distance*smooth + (1-smooth)*(m.getArgAsFloat(7));
                }
...
void thawAppManager::draw(){
    if(isCalibrating){
        ...
        if(fullscreenpattern){
            if(showPattern)drawFullScreenPattern();
        }
        else if(onScreenMode && !onTrackPattern){
            if(pattern_size > ofGetScreenWidth() && showPattern) drawFullScreenPattern();
            else if (showPattern)drawPartScreenPattern(phoneX+90, phoneY+70, pattern_size *1.4);
        }
        else if(onScreenMode){
            float size = 20*sqrt(phoneVX*phoneVX + phoneVY*phoneVY);
            if(size > 300.f) size = 300.f;
            if(size< 100.f) size = 100.f;
            if(showPattern) drawPartScreenPattern(phoneX+90, phoneY+70, size);
        }
        ...
        ofDrawBitmapString("params: " + ofToString(axr) + ", " + ofToString(axg)+ ", " + ofToString(byr) + ", " + ofToString(byg), 800, 20);
    }
}
...
void thawAppManager::drawPartScreenPatternAfter(){
    
    ofSetCircleResolution(128);
    
    if(onScreenMode && !onTrackPattern){
        if(pattern_size > ofGetScreenWidth()) drawFullScreenPattern();
        else drawPartScreenPattern(phoneX+60, phoneY+70, pattern_size *1.4);
    }
    else if(onScreenMode){
        float size = 20*sqrt(phoneVX*phoneVX + phoneVY*phoneVY);
        if(size > 300.f) size = 300.f;
        if(size< 100.f) size = 100.f;
        drawPartScreenPattern(phoneX+60, phoneY+70, size);
    }
}
...
void thawAppManager::drawPartScreenPattern(int x, int y, int size){
    ...
    float sz = pattern_size * min(1.5f,1 + max(0.f,distance-50)/6.f);
...
void thawAppManager::drawPartScreenPatternAfter(int x, int y, int w, int h){
